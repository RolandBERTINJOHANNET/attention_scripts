...
def apply_corruption(latents, shape_info, scale, device):
    ...
    # Batch size and split index
    batch_size = v_latents_tensor.size(0)
    split_index = batch_size // 2

    latent_dim = v_latents_tensor.shape[1]  # Assuming latent_dim is the second dimension of v_latents_tensor

    # Generate a batch of random normal corruption vectors for each half
    corruption_vectors_v_latents = torch.randn(split_index, latent_dim).to(device)
    corruption_vectors_v_latents = (corruption_vectors_v_latents - corruption_vectors_v_latents.mean(dim=1, keepdim=True)) / corruption_vectors_v_latents.std(dim=1, keepdim=True) * scale

    corruption_vectors_attr = torch.randn(batch_size - split_index, latent_dim).to(device)
    corruption_vectors_attr = (corruption_vectors_attr - corruption_vectors_attr.mean(dim=1, keepdim=True)) / corruption_vectors_attr.std(dim=1, keepdim=True) * scale

    # Apply corruption to the first half of v_latents and the second half of attr
    v_latents_corrupted = v_latents_tensor[:split_index] + corruption_vectors_v_latents
    attr_corrupted = attr_tensor[split_index:] + corruption_vectors_attr
...
def process_batch(scale, scale_key, latents, shape_info, current_epoch_metrics, device, attention_mechanism, module, classification_head, criterion):
    latents_v_latents_corrupted, shape_info_v_latents, latents_attr_corrupted,shape_info_attr = apply_corruption(latents, shape_info, scale, device)
