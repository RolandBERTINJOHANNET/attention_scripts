...
model_filename = os.getenv('MODEL_FILENAME_2')
...
attention_model_checkpoint_path = f'models/{fusion_model_name}/{model_filename}'
...
# Retrieve the corruption vector from the checkpoint (assuming it exists in the checkpoint)
corruption_vector = attention_checkpoint['corruption_vector']
        ...
        for corruption_target_domain in ['v_latents', 'attr']:
            ...
            for domain in latents_corrupted.keys():
                ...
                if domain_key == corruption_target_domain:
                    print("corruption vector : ",corruption_vector[:5])
                    latents_corrupted[domain][domain_key] += scale * corruption_vector
...
plot_dir = f"scoreplots_contrastives/scoreplots_{fusion_model_name}_{model_filename}"
